...
// ---------------------------------------------------------------------------
// V-VAL Phase 5 addendum.
//
// The two tests above FAILED on their first execution, but the failure is a
// FALSE POSITIVE in the PoC's own needle, not a leak. `KeyShare::dummy()` sets
// the signing share to `Scalar::ONE` AND the identifier to
// `Identifier::try_from(1)`. Both serialize to the same 32-byte big-endian
// `0000…0001`, and frost-core prints the identifier in the clear (it is public
// data). So `rendered.contains(ONE_BE_HEX)` matches the *identifier*, while the
// actual rendering reads `signing_share: SigningShare("<redacted>")`.
//
// This test removes the ambiguity by isolating the `signing_share` field of the
// rendering before searching it.
// ---------------------------------------------------------------------------

/// Extracts the substring rendered for the named field, up to the next `,` at
/// the same nesting depth.
fn field_rendering<'a>(rendered: &'a str, field: &str) -> &'a str {
...
#[test]
fn signing_share_field_alone_is_redacted() {
    let effect = Effect::StartNonceGeneration {
        group_id: GROUP,
        key_share: Arc::new(KeyShare::dummy()),
    };
    let rendered = format!("{effect:?}");
    let signing_share = field_rendering(&rendered, "signing_share");
    let identifier = field_rendering(&rendered, "identifier");
    println!("\n=== isolated fields ===");
    println!("identifier    -> {identifier}");
    println!("signing_share -> {signing_share}");
    for needle in leaky_substrings() {
        assert!(
            !signing_share.contains(&needle),
            "the signing_share field itself renders {signing_share} which contains {needle:?}"
        );
    }
    assert_eq!(
        signing_share, "SigningShare(\"<redacted>\")",
        "frost-core 3.0.0 is expected to redact SigningShare in its Debug"
    );
    // The identifier — public data — is what the original needle was matching.
    assert!(
        identifier.contains(ONE_BE_HEX),
        "the original false positive came from the identifier; expected it here"
    );
}
