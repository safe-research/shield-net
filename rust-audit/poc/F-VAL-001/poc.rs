...
fn impostor_share_verifies_and_group_finalizes() {
    ...
    let sharing_states_for_truth = c.sharing_states.clone();
    let Ceremony {
        published,
        verified,
        mut sharing_states,
    ...
    let mut verified_shares: BTreeMap<Address, BTreeMap<Address, keygen::VerifiedShare>> =
        BTreeMap::new();
    // V-VAL Phase 5 MECHANICAL REPAIR (does not change what is asserted).
    //
    // The first execution of this loop panicked with
    //   `0x976E… (IMPOSTOR) rejected 0xf39F… (a BYSTANDER)'s share`.
    // That is NOT the assertion this test is about. It is the *impostor* failing
    // to decrypt an *honest* peer's ciphertext, which is the direct and expected
    // consequence of the attack itself: having published `q_A` in place of its
    // own `q_M`, every honest peer encrypts to `q_A`, and
    // `verify_encrypted_secret_share` decrypts with the impostor's own `sk_M`.
    // A real attacker does not run that code path — it decrypts with the pads it
    // harvested (asserted below), and simply sends `keyGenConfirm` regardless,
    // because nothing onchain checks that it verified anything.
    //
    // The claim under test — "every HONEST peer accepts the impostor's forged
    // share and no honest peer has grounds to complain" — is about the six
    // honest holders, so the holder loop now iterates them.
    ...
    // Every honest participant finalizes, including the victim, and they agree
    // on one group key.
    ...
    assert_eq!(finalized, 6, "all six honest participants finalized");
    assert!(!group_key.x.is_zero(), "the group produced a key");

    // And the impostor is not locked out either: it recovers every honest
    // peer's share to it using the pads it harvested, so it can derive its own
    // key share out of band. Ground truth is each peer's own plaintext.
    for peer in PARTICIPANTS {
        if peer == IMPOSTOR {
            continue;
        }
        let ciphertext = published[&peer].f[slot(peer, IMPOSTOR)];
        let recovered = xor(ciphertext, pads[&peer]);
        let ground_truth =
            keygen::reveal_secret_share(&sharing_states_for_truth[&peer], IMPOSTOR).unwrap();
        assert_eq!(
            recovered, ground_truth,
            "the impostor decrypts {peer}'s share to it with the harvested pad"
        );
    }
}
...
fn impostor_recovers_victim_signing_share() {
    ...
    // Every honest participant finalizes (Trigger step 8). The impostor is
    // skipped for the reason documented in `impostor_share_verifies_and_group_finalizes`:
    // it cannot run `verify_encrypted_secret_share` with `sk_M` after publishing
    // `q_A`, and does not need to — it decrypts with the harvested pads. The
    // ground truth this test compares against is the VICTIM's key share, which
    // is produced by the honest path below.
