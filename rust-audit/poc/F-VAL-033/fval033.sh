...
TMPDIR="$SC/f033_tmp"; rm -rf "$TMPDIR"; mkdir -p "$TMPDIR"
PIDS=(); VA_PID=""; VB_PID=""
cleanup(){ for p in "${PIDS[@]:-}"; do kill "$p" 2>/dev/null; done; }
trap cleanup EXIT

echo "==> anvil on $RPC"; anvil --block-time "$BLOCK_TIME" --port "$ANVIL_PORT" > "$SC/f033_anvil.txt" 2>&1 & PIDS+=("$!")
...
start_va(){ "$REPO_ROOT/target/debug/validator" --config-file "$TMPDIR/va.toml" >> "$SC/f033_valA.txt" 2>&1 & VA_PID="$!"; PIDS+=("$VA_PID"); }
start_vb(){ "$REPO_ROOT/target/debug/validator" --config-file "$TMPDIR/vb.toml" >> "$SC/f033_valB.txt" 2>&1 & VB_PID="$!"; PIDS+=("$VB_PID"); }
: > "$SC/f033_valA.txt"; : > "$SC/f033_valB.txt"
