...
Line counts measured this session with `wc -l`. Every file below was read **100%**, including its
test module.
...
Read partially, to check whether a core-level defect is reachable or already mitigated downstream.
Any claim I make about these is cited as a *related* location, never as the mechanism.
...
Not filed as findings but recorded below: 4 rejected leads with citations (section 4) and 11
observations (section 5).

## 4. Hypotheses considered and rejected

One entry per lead from my launch brief, the map and the analysis, plus my own ideas. Each says what
refuted it.
...
| **CORE-H13** (wall-clock polling) | **Deferred to R1, with the mechanism confirmed by reading.** Everything is in R1's files: `index/clock.rs:31-38` (`SystemTime::now`), `index/blocks.rs:538-543` (`timestamp * 1000 + block_time`), `:548-551`. Note the harm is one-sided and self-limiting: `sleep_until` returns immediately when the target is past (`clock.rs:52-57`), so a clock *ahead* of chain time costs only a retry, and a clock *behind* by Δ delays polling by Δ. With working NTP, Δ is milliseconds. I file nothing to avoid duplicating R1. |
...
| **Core checklist 11** (timeouts and shutdown; no partial state observable on kill) | **Answered.** F-CORE-039 for the shutdown bound. On "no partial state": a kill between the snapshot commit (`state/mod.rs:236`) and the action enqueue (`driver.rs:277`) *is* observable, but is repaired by the restart replay of the retained window (`blocks.rs:255-266`) — at the cost of duplicates (CORE-H5). The unrepairable partial state is the resume half: F-CORE-031. |
...
| 5 | `metrics::rpc_requests_total`'s `method` label is attacker-influenced → unbounded Prometheus cardinality. | Refuted. The label is taken from the *request* we construct, not from any response: `request.method.to_owned` (`provider/mod.rs:70-76`), and the response only flips the `result` label (`provider/mod.rs:95-114`). Cardinality is bounded by the ~8 methods core calls. |
...
| 7 | The double `initialize` in the validator (`observability::init` → `crate::metrics::initialize`, then `main.rs:42`) spawns the tokio-metrics reporter twice. | Refuted: `main.rs:42` calls the *validator's* `metrics::initialize`, a different module; core's runs once. |
...
| --- | --- | --- | --- |
| O-1 | **State-machine poisoning.** `handle_update`/`handle_resume` `mem::take` the state before doing fallible work, so *any* error — including the `_ => return Err(Error::BadUpdate)` arm, which returns before restoring — leaves `inner = None` and every later call returns `Poisoned`. The state value is dropped, so no recovery is possible even in principle. | `state/mod.rs:170-171`, `:240-242`, `:252` | Moot for the only caller: the driver exits on the first error. It is a latent hazard for any other consumer of this `pub` API, and it makes a "retry the update" fix impossible without restructuring. |
...
- Every one of the twelve assigned files was read end to end, including test modules. Nothing was
  skipped, sampled or skimmed.
- Nothing was executed: no `cargo`, no tests, no scripts, no network (A9 FALSE — see
  `state/baseline.md` §2). Every basis row in my findings is `E2` or `I`; none is `E1`, and none
  claims 90%+.
- Files outside my scope are cited only as *related* locations, and section 1 lists exactly which
  parts of them I read. Where a mechanism lives in another reviewer's file (CORE-H13 in `index/`,
  the duplicate-action half of CORE-H5 in `tx/`), I recorded the analysis in section 4 and did not
  file a competing finding.
- Two claims in my findings rest on dependency behaviour that is not on disk and are marked `I`:
  what `frost-core`'s `Debug` prints (F-CORE-036) and whether an un-timed HTTP request can hang
  indefinitely (F-CORE-039). Both are one command away for an agent with a toolchain.
