...
| H-R6-2 | Two events across the three watched contracts might share a `topic0`, so `watcher_events!`'s first-match-wins decode (`core/index/events.rs:577-590`) could misdecode one as the other. | Same computation: no `topic0` collisions among the 18 events and no 4-byte collisions among the 18 selectors. Topic list recorded in the finding F-VAL-060's *Considered and rejected*. |
