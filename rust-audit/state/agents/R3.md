...
Role: Reviewer R3. Scope: `crates/core/src/tx/` (5 files, 1,540 lines). Finding ID range `F-CORE-060`…`F-CORE-089`.
Run mode: READ-ONLY, no Rust toolchain (A9 FALSE). No `cargo` was run. Best basis class attainable is `E2`.
Commit: `2893917`.
...
| F-CORE-061 | `is_transaction_underpriced` only matches *replacement* rejections; a first-submission fee rejection retries at an unchanged fee forever and blocks every later nonce | Medium | 60% |
...
1. **CORE-H6 (fee bumps compound without a ceiling; the cap does not apply to bumps)** — **CONFIRMED and escalated**, filed as `F-CORE-060`. The analysis put the rate at "one bump per `blocks_before_resubmit` blocks" and rated it 55%/Medium. That understates it: an *underpriced* rejection records `submitted_at = NULL` (`tx/mod.rs:277-282` passes `block: None`; `tx/storage.rs:174-196` binds it as SQL `NULL`), and `stale_submissions` selects `submitted_at IS NULL` unconditionally (`tx/storage.rs:296-297`), so such a transaction is rebuilt **every** advancing block, not every second block. The crate's own test proves the per-block rate: fees go 231/11 at block 12 to 255/13 at block 13 (`tx/mod.rs:703-717`).

2. **CORE-H7 (first-submission "transaction underpriced" and non-geth messages unrecognised)** — **mechanism CONFIRMED**, filed as `F-CORE-061`. The regex pair at `tx/mod.rs:362-368` requires "replacement transaction" *and* "underpriced", so any message lacking the word "replacement" takes the generic branch at `tx/mod.rs:287-293`, which does not call `record_submission`; the row is then rebuilt with `bump(fresh, None) == fresh` (`types.rs:64-66`, `fees.rs:39-42`). The *exact wording* other clients emit cannot be verified offline (no dependency or client sources on disk), so that half stays class `I` and the finding is scoped to the code path, not to a named client string.
...
15. **Startup reconciliation being skipped because `self.block_status` is assigned before the fallible work** — **rejected.** `self.block_status = Some(status)` at `tx/mod.rs:153` does precede `prune`/`unmark_executed`, so an early failure would move `previous` off `None` and permanently skip the `None => status.safe.checked_add(1)` branch (`tx/mod.rs:167-179`). But every statement between line 153 and the `unmark_executed` call is local or SQLite, and a SQLite error is *not* intermittent (`tx/mod.rs:47-54`), so the driver exits (`driver.rs:248`) rather than continuing with a half-applied startup. The first RPC call (`nonce`, line 188) comes *after* `unmark_executed`. So the ordering is safe by construction, though only accidentally.

16. **An intermittent RPC failure at `nonce` losing a block's reconciliation permanently** — **rejected.** The failure returns `Error::Rpc`, which the driver logs and swallows (`driver.rs:247-253`). `self.block_status` is already advanced, but the gate at `tx/mod.rs:185` is `previous.latest < status.latest`, so the *next* advancing block re-runs `mark_executed`/`resubmit_stale`/`submit_pending`. The loss is one block of latency, not permanent.
...
26. **A reorg failing to unmark executed transactions** — **rejected for the observable case.** `BlockWatcher::status` derives `latest` from `pending.number - 1` (`index/blocks.rs:376-381`), and the uncle path rewinds `pending`, so `latest` really does regress and `tx/mod.rs:174` fires `unmark_executed(latest + 1)`. The case that *is not* covered — the account nonce regressing without a block-height regression — is `F-CORE-063`.
