...
`crates/core/Dockerfile` does not exist (`core` is a library crate) — confirmed against baseline §4,
not reported as missing.

### Also read in full (other reviewers' files; read for the sweeps, no findings filed on them alone)

`crates/validator/src/config.rs` (290), `crates/sentinel/src/config.rs` (144),
`crates/sentinel-engine/src/config.rs` (154), `crates/core/src/observability/mod.rs` (94),
`crates/core/src/observability/logging.rs` (21), `crates/core/src/observability/metrics.rs` (80),
`crates/core/src/metrics.rs` (90), `crates/validator/src/metrics.rs` (132),
`crates/sentinel/src/metrics.rs` (134), `crates/core/src/effects.rs` (220),
`crates/core/src/kdf.rs` (81), `crates/sentinel-engine/src/api/mod.rs` (60),
`crates/sentinel-engine/src/api/extractors.rs` (69), `crates/sentinel-engine/src/main.rs` (87),
`crates/validator/src/frost/error.rs` (46), the three `Dockerfile.dockerignore` files (4 each).

### Read in part (specific ranges, for citation)

`crates/validator/src/service/effect.rs:1-130, 235-276`; `crates/validator/src/frost/keygen.rs:20-120, 425-530`;
`crates/validator/src/frost/ecdh.rs:1-20` + grep of the whole file; `crates/validator/src/frost/preprocess.rs:30-70`;
`crates/core/src/driver.rs:20-70, 236-263`; `crates/core/src/provider/mod.rs:55-166`;
`crates/core/src/index/mod.rs:14-45`; `crates/core/src/index/blocks.rs:16-50`;
`crates/core/src/tx/mod.rs:60-115, 340-370`; `crates/core/src/state/storage.rs:45-75`;
`crates/core/src/tx/storage.rs:65-95`; `crates/validator/src/secrets/store.rs:60-95`;
`crates/validator/src/main.rs:30-100`; `crates/sentinel/src/engine.rs:100-200`;
`crates/sentinel-engine/src/checkers/address_poisoning.rs:150-215, 240-290, 300-385`.

### Reference material (no findings)

`rust-audit/state/reviewer-brief.md` (76, 100%), `rust-audit/state/baseline.md` (404, 100%),
`rust-audit/PROMPT.md` (231, 100%), `rust-audit/codebase-map.md` §1-5, §7-10 (~150 of 393 lines),
§8 of all four analysis files (100% of each §8), plus the "considered and rejected" section of
each analysis. `Justfile` (169, first 80 lines read; the rest is integration recipes),
`.github/workflows/ci.yml` (39, 100%), `.github/workflows/docker.yml` (70, 100%),
`.github/workflows/integration.yml` (first 40 lines), `docs/configuration.md` (83, 100%).

**Not read:** `crates/sentinel-engine/openapi.yaml` (R8's), the four analysis files outside §8 and
their rejected lists, `docs/validator-handbook.md`, `docs/sentinel-handbook.md`,
`docs/sentinel-engine.md`, `docs/overview.md`, `contracts/src`, the `epics/` folder. The three
handbooks are cited indirectly through the samples' own comments; I did not verify those
cross-references line by line, which is a gap the Coverage Critic should note against F-XC-009.
...
Scratchpad artefacts (session-local, will not survive):
`.../scratchpad/r10/census.py`, `.../scratchpad/r10/census.txt`, `.../scratchpad/r10/testranges.txt`.

## 4. Consolidated panic and cast census (sweep 2) — re-derived, not copied

Method: a Python scanner (`census.py`) that first computes, per file, the line ranges belonging to
a `#[cfg(test)]` item by brace matching, then greps the **remaining** lines for `unwrap`,
`unwrap_err`, `expect(`, `panic!`, `unreachable!`, `todo!`, `unimplemented!`, `assert*!`,
`as <intty|floatty>`, indexing, slicing and `unsafe`, skipping comment-only lines. Run over all 83
`.rs` files. This replaces, and is independent of, the four analyses' §8 tables.
...
**Conclusion: no attacker-reachable panic exists in non-test code.** `cow.rs:130` is the only
site whose safety rests on a caller-set invariant rather than on the local types, and it is R9's.
**No consolidated panic finding was filed** — filing one would inflate a clean result.
...
None of the four censuses overstates. The one systematic weakness is the validator's, which lists
sites selectively rather than mechanically.

## 5. Dependency questions answered, and the ones that remain unanswerable (sweep 3)

**Answered statically:**

- *Are the two SHA-256 stacks ambiguous at any parity-critical call site?* **No.** Every hash that
  must match Solidity is `alloy::primitives::keccak256`. The only two SHA-256 uses name their
  stack by import path: `k256::sha2::Sha256` (0.10.9) at `validator/frost/ecdh.rs:13,125` for
  hash-to-field, and plain `sha2::Sha256` (0.11.0) at `core/kdf.rs:5,22` under HKDF, whose output
  is the sentinel's local reveal salt and never a consensus value. Recorded in F-XC-007 basis
  rows 9-10 so it is not re-opened.
- *Does the validator's RNG share a `rand_core` line with FROST?* **Yes** — `rand 0.8.6`,
  `frost-core 3.0.0` and `frost-secp256k1 3.0.0` all take `rand_core 0.6.4`, as do
  `elliptic-curve 0.13.8`, `ff`, `group`, `signature` and `crypto-bigint`. Independently proven by
  the fact that `keygen::setup`'s `&mut R` is passed straight into `dkg::part1`. Baseline §6
  confirmed.
- *Features that widen surface:* `alloy` `full` (ws/ipc/signer-local/tungstenite all in the lock),
  `tokio` `full` (includes `process`, unused), `sqlx` default features (pull `sqlx-mysql` and
  `sqlx-postgres` into the graph although only SQLite is used and no compile-time macro exists
  anywhere). Filed as F-XC-007.
- *A dependency for a trivial reason:* `regex` in `core`, used for three literal case-insensitive
  substrings at `tx/mod.rs:362-367`. Filed as F-XC-007.
- *Lockfile hygiene:* committed, format 4, and `--locked` is used by the clippy gate and all three
  Dockerfiles. No drift possible unnoticed. Not a finding.
...
2. Whether `serde_derive` accepts, rejects, or silently ignores `deny_unknown_fields` on a
   container with a `flatten` field, in the locked version (F-XC-003's open leg).
3. Whether `frost-core`'s `round1::SecretPackage` and `keys::KeyPackage` redact in `Debug`
   (F-XC-002's open leg) — the single highest-value question in this list.
4. Whether the linker actually strips the unused `sqlx-mysql`/`sqlx-postgres` code from the
   release binaries, i.e. whether F-XC-007 item 2 is a real size/surface reduction or only a
   build-time one.
5. `reqwest 0.13.4`'s actual default redirect and proxy policy (F-XC-008's basis row 8).
6. Which TLS root source `reqwest` selects under `default-features = false, features =
   ["json", "rustls"]`, and therefore whether the runtime images' `ca-certificates` is load-bearing.
7. Whether `cargo clippy -D warnings` currently passes — CI says it must, but nothing here
   observed it.
8. Whether `cargo tree -d`'s real output matches the lockfile-derived duplicate list in
   baseline §6 (the substitute was a text parse, and it cannot see feature-gated edges).
...
| Lead | Outcome |
| --- | --- |
| **VAL-H10** (secret structs derive `Debug`, logged at `warn`) | **Promoted and broadened** into F-XC-002. The analysis is right about `service/effect.rs:249` but stops there; the generic `core` sites (`effects.rs:55,59,78`, `driver.rs:238,261`) log every `Effect`, `Resume` and `Update` of *any* service, which is the cross-cutting form. The analysis' 25% confidence understates the *pattern* and correctly states the *leak*. Also corrected: the analysis implies all secret-bearing types derive `Debug`; four hand-write a redacting impl (`ecdh.rs:50`, `preprocess.rs:64`, `:150`, `signer.rs:96`) and exactly two do not. |
| **SEN-H14** (`reqwest::Client::new` defaults) | **Promoted and broadened** into F-XC-008. Confirmed at `engine.rs:113` and found a second, worse instance at `sentinel-engine/checkers/cow.rs:230` which additionally has *no timeout*, where the sentinel's does (`engine.rs:155`). |
...
| The zero-address sample values start a misconfigured service silently | They do not: `consensus = 0x0` makes `Consensus::getCoordinator` fail to decode and `main` returns `Err` (`validator/src/main.rs:49-53`). This is the contrast that makes the *valid* placeholders (the key, the zero salt) the real problem. |
...
1. **Build-context breadth.** All three `Dockerfile.dockerignore` files are `/**` with
   `!/Cargo.toml`, `!/Cargo.lock`, `!/crates/**`, so everything under `crates/` enters the build
   context and the builder layer — including a real `validator.toml` or a `.sqlite` file a
   developer left beside the sample. The builder layer is not pushed, so this is not exposure
   today; a `!/crates/*/src/` + `!/crates/*/Cargo.toml` allowlist would be tighter.
2. **The sentinel-engine exports no metrics of its own.** There is no `metrics.rs` in the crate
   and no `metrics::` call outside `safenet-core`, so an operator has no counter for checker
   outcomes, verdict mix, or CoW/RPC failures. This is why F-XC-005's degradation is invisible.
   Probably R8's to file properly.
3. **`ca-certificates` may be dead weight.** If `reqwest 0.13` under `rustls` uses bundled
   `webpki-roots` (both 0.26.11 and 1.0.7 are in the lock), the runtime images' `ca-certificates`
   package is unused *and*, more importantly, adding a private CA to the system trust store would
   not work. Unverifiable without the dependency source (A6).
4. **`reqwest::Error`'s `Display` includes the request URL** (`sentinel/engine.rs:182`
   `tracing::error!(%err, ..)`). If an operator ever puts credentials in the engine URL, they may
   reach the log. Whether reqwest redacts userinfo is upstream and unverified.
5. **Axum's default `JsonRejection` body** echoes the serde error, which can include a field path
   and sometimes a value fragment, on a malformed `CheckRequest`. Under A3 the caller is the
   trusted sentinel, so Informational at most.
6. **`validator/src/service/action.rs:154`** computes `250_000 + 25_000 * share.f.len as u64`
   as a gas limit. It cannot overflow `u64` (the validator analysis is right), but at the `u16`
   bound it yields ~1.6 billion gas, far above any block gas limit, so the transaction would be
   unmineable rather than merely expensive. R6's to assess.
7. **No `EXPOSE` and no `HEALTHCHECK`** in any image; noted inside F-XC-004 rather than filed
   separately.
8. **`blocklist = []`** in the engine sample means the blocklist checker never denies. Documented
   as deliberate in the sample itself and inherently deployment-specific, so not filed.
9. **`tokio-metrics` spawns a background reporter** at `core/src/metrics.rs:89` inside
   `observability::init`. It panics outside a runtime; all three binaries call it inside
   `#[tokio::main]`. Safe today, fragile if `init` is ever called earlier. R2's.
10. **The map's §9 line totals for R2 and R9 are wrong** (1,993 vs 1,998 and 3,275 vs 3,471), as
    baseline §3 already records. Repeated here only so the Coverage Critic sees it corroborated.

## 8. Gaps and blockers

- No toolchain: `E1` is unreachable, and the eight questions in §5 stay open. Every finding I
  wrote states which of its legs is `I` for this reason.
- I did not read the three service handbooks, which several sample-config comments point at. If a
  handbook already warns about the placeholder key or the `0.0.0.0` bind, F-XC-009's severity
  should drop; a Critic with budget should check `docs/validator-handbook.md` "Secrets" and
  `docs/sentinel-engine.md` before finalising it.
